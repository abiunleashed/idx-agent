...
1. DURUM: (Ozet)
1. RISK: (Kritik seviye)
1. KARAR: (AL/SAT/BEKLE + gerekce)
...
body{background:#080810;color:#e0e0e0;font-family:'Courier New',monospace;height:100vh;display:flex;flex-direction:column;overflow:hidden}
.hdr{background:linear-gradient(135deg,#0d1117,#161b27);border-bottom:2px solid #c9a227;padding:12px 20px;display:flex;align-items:center;justify-content:space-between;flex-shrink:0}
...
.pill{background:#0d1a2a;border:1px solid #1e3a5f;border-radius:20px;padding:5px 12px;font-size:11px;color:#4fc3f7}
...
.ticker{background:#0a0a14;border-bottom:1px solid #1a1a2e;padding:6px 20px;display:flex;gap:18px;overflow-x:auto;flex-shrink:0;scrollbar-width:none}
...
.sig-bar{background:#0a140a;border-bottom:1px solid #1a2e1a;padding:6px 20px;font-size:11px;color:#4caf50;min-height:26px;flex-shrink:0;overflow:hidden;text-overflow:ellipsis;white-space:nowrap}
.main{display:flex;flex:1;overflow:hidden}
.sidebar{width:250px;background:#0c0c18;border-right:1px solid #1a1a2e;padding:13px;overflow-y:auto;flex-shrink:0}
.sec{margin-bottom:18px}
.sec-t{font-size:10px;color:#c9a227;letter-spacing:2px;text-transform:uppercase;margin-bottom:8px;border-bottom:1px solid #2a2a3e;padding-bottom:4px}
.pi{display:flex;justify-content:space-between;align-items:center;padding:5px 0;border-bottom:1px solid #121228}
.psym{color:#c9a227;font-size:12px;font-weight:bold}
...
.qbtn{width:100%;background:#0f0f1e;border:1px solid #252540;color:#c9a227;padding:7px 9px;margin-bottom:4px;border-radius:5px;cursor:pointer;font-size:11px;text-align:left;transition:all .2s;font-family:'Courier New',monospace}
.qbtn:hover{background:#c9a227;color:#000}
.chat{flex:1;display:flex;flex-direction:column;overflow:hidden}
.msgs{flex:1;overflow-y:auto;padding:16px;display:flex;flex-direction:column;gap:12px}
.msg{display:flex;gap:9px;max-width:90%}
.msg.user{align-self:flex-end;flex-direction:row-reverse}
.av{width:32px;height:32px;border-radius:50%;display:flex;align-items:center;justify-content:center;font-size:15px;flex-shrink:0}
.av.wolf{background:linear-gradient(135deg,#c9a227,#7a5f10)}
.av.user{background:linear-gradient(135deg,#1e3a5f,#0d2137);font-size:11px;color:#4fc3f7;font-weight:bold}
.bbl{padding:10px 13px;border-radius:9px;font-size:13px;line-height:1.7}
...
.typing-bbl{padding:9px 13px;background:#0c1622;border:1px solid #1e3a5f;border-left:3px solid #c9a227;border-radius:9px;display:flex;gap:5px;align-items:center}
.tdot{width:6px;height:6px;border-radius:50%;background:#c9a227;animation:typ 1.4s infinite}
.tdot:nth-child(2){animation-delay:.2s}.tdot:nth-child(3){animation-delay:.4s}
@keyframes typ{0%,80%,100%{opacity:.3;transform:scale(.8)}40%{opacity:1;transform:scale(1)}}
.inp-area{padding:11px 16px;background:#0c0c18;border-top:1px solid #1a1a2e;display:flex;gap:8px;flex-shrink:0}
.inp{flex:1;background:#0f0f1e;border:1px solid #252540;border-radius:7px;padding:9px 12px;color:#e0e0e0;font-size:13px;font-family:'Courier New',monospace;outline:none;transition:border-color .2s}
.inp:focus{border-color:#c9a227}
.inp::placeholder{color:#2a2a3a}
.sbtn{background:linear-gradient(135deg,#c9a227,#7a5f10);border:none;border-radius:7px;padding:9px 15px;color:#000;font-weight:bold;cursor:pointer;font-size:14px;transition:transform .2s}
.sbtn:hover{transform:scale(1.05)}
.sbtn:disabled{opacity:.4;cursor:not-allowed;transform:none}
.intro{background:linear-gradient(135deg,#0c1622,#0f1a2e);border:1px solid #c9a227;border-radius:9px;padding:14px}
.intro-t{color:#c9a227;font-size:13px;font-weight:bold;margin-bottom:6px}
.intro-b{color:#777;font-size:12px;line-height:1.7}
.risk{background:#120800;border-top:1px solid #2a1400;padding:4px 20px;font-size:10px;color:#4a2800;text-align:center;flex-shrink:0}
...
<div class="ticker" id="tickerBar"><span class="neu">Piyasa yukleniyor... (ilk yuklemede 30sn bekleyin)</span></div>
<div class="sig-bar" id="sigBar">Sinyaller hesaplaniyor...</div>
<div class="main">
  <div class="sidebar">
    <div class="sec">
      <div class="sec-t">Portfoy</div>
      <div class="pi"><div><div class="psym">BBCA</div><div class="pname">Bank Central Asia</div></div><span class="pw">%25</span></div>
      <div class="pi"><div><div class="psym">DCII</div><div class="pname">DCI Indonesia</div></div><span class="pw">%20</span></div>
      <div class="pi"><div><div class="psym">ICBP</div><div class="pname">Indofood CBP</div></div><span class="pw">%15</span></div>
      <div class="pi"><div><div class="psym">TLKM</div><div class="pname">Telkom Indonesia</div></div><span class="pw">%15</span></div>
      <div class="pi"><div><div class="psym">BMRI</div><div class="pname">Bank Mandiri</div></div><span class="pw">%5</span></div>
      <div class="pi"><div><div class="psym">AMRT</div><div class="pname">Alfamart</div></div><span class="pw">%8</span></div>
      <div class="pi"><div><div class="psym">INDF</div><div class="pname">Indofood</div></div><span class="pw">%2</span></div>
    </div>
    <div class="sec">
      <div class="sec-t">Radar</div>
      <div class="pi"><div><div class="psym">MYOR</div><div class="pname">Mayora/Kopiko</div></div><span class="pr">Ekle?</span></div>
      <div class="pi"><div><div class="psym">SIDO</div><div class="pname">Sido Muncul</div></div><span class="pr">Ekle?</span></div>
      <div class="pi"><div><div class="psym">ACES</div><div class="pname">Ace Hardware</div></div><span class="pr">Ekle?</span></div>
      <div class="pi"><div><div class="psym">BREN</div><div class="pname">Barito Renewables</div></div><span class="pw2">Izle</span></div>
    </div>
    <div class="sec">
      ...
      <button class="qbtn" onclick="q('ICBP 6 Mayis kazanc oncesi ne yapmaliyim?')">ICBP Kazanc</button>
      <button class="qbtn" onclick="q('MYOR ve SIDO portfoyume eklemeli miyim?')">Yeni Hisse</button>
      <button class="qbtn" onclick="q('Portfoyumun risk haritasini cikar')">Risk Analizi</button>
    ...
    <div class="msgs" id="msgs">
      <div class="intro">
        ...
        <div class="intro-b">
          ...
          <strong style="color:#c9a227">600 juta IDR | Buffett Stratejisi | DCA 7.5 juta/ay</strong><br>
          <span style="color:#555;font-size:11px">Ilk veri yuklemesi 30sn surebilir.</span>
        </div>
      </div>
    </div>
    <div class="inp-area">
      <input class="inp" id="inp" placeholder="Wolf'a sor..." onkeypress="if(event.key==='Enter')send()"/>
      <button class="sbtn" id="sbtn" onclick="send()">&#x27A4;</button>
    </div>
  </div>
</div>
<div class="risk">Wolf yatirim tavsiyesi vermez. Tum kararlar size aittir. | Stop-loss: -%12 | 600 juta IDR</div>
<script>
var hist=[], sending=false;

function tick(){
var w=new Date(new Date().toLocaleString(“en-US”,{timeZone:“Asia/Jakarta”}));
document.getElementById(‘clk’).textContent=‘WIB ‘+w.getHours().toString().padStart(2,‘0’)+’:’+w.getMinutes().toString().padStart(2,‘0’);
}
...
if(role===‘wolf’){
d.innerHTML=’<div class="av wolf">🐺</div><div class="bbl wolf"><div class="wh">WOLF - ‘+t+’</div>’+content.replace(/\n/g,’<br>’)+’</div>’;
} else {
...
d.innerHTML=’<div class="av wolf">🐺</div><div class="typing-bbl"><div class="tdot"></div><div class="tdot"></div><div class="tdot"></div></div>’;
